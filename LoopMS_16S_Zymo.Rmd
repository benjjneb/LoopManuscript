...
get.ham <- function(sq, ref, window=40, step=10) {
...
assign.ref <- function(sqi, refs) {
  hams <- sapply(refs, function(ref) get.ham(sqi, ref=ref))
...
assignments <- lapply(sq10.incorrect, assign.ref, refs=refs)
...
library(ggplot2)
ggplot(data=df, aes(x=Position, y=Assignment, color=Assignment)) + geom_point() + facet_wrap(~Sequence)


# Need to do ~1000 * 27 of these... time no problem, so about 27 seconds. Time no problem.
#foo <- RNAStringSet(DNAStringSet(dd$sequence))
#al <- AlignSeqs(foo)
#bar <- stringDist(narrow(al, 1, 50), method="hamming") # Letter mismatches, which is what I want
```

These results coincide quite well with the manual BLAST inspections, and suggest it would be possible to systematically scan for chimera/introgression errors. Another question is how effectively a simple approach of optimizing the quality score filtering can remove these errors:
