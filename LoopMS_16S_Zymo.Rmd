...
color.scale <- c("Bimera"="#a6cee3", "Introgression"="#1f78b4", 
...
ggsave(file.path(path.fig, "ErrorsByTypeAndMaxEE.pdf"), pbar,
                 width=8, height=4, units="in", useDingbats=FALSE)
```

Officially recommend a `maxEE=0.5` filter for Loop data. At a loss of ~10% of reads, reduces structural errors to <1% of total reads, and total reads with errors to ~4% of total reads.

Implement this recommendation, and evaluate high-sensitivity DADA2:
```{r}
filt.opt <- file.path("filtered", "optimized", basename(fn))
...
```{r}
foo <- lapply(dd.opt$sequence[28:length(dd.opt$sequence)], assign.ref)
```
